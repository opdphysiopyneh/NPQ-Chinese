...
<head>
  ...
  <style>
    ...
    legend.question-title { font-weight:bold; margin-bottom:8px; padding:0 6px; float:left; width:100%; }
...
<div class="container">
  ...
  <div id="warning" class="warning" dashed aria-live="assertive"></div>
</div>
...
<script>
const questions = [
  { title: "1. 現在頸痛的程度", options: ["沒有頸痛", "溫和", "中等", "很厲害", "簡集不可想像"] },
...
function calculateNPQ() {
  ...
  for (let i = 1; i <= 9; i++) {
    ...
    if (!selected) {
      if (isOptional) {
        // Skip Question 9 from calculation entirely if omitted
        continue;
      } else {
    ...
    maxPossibleScore += 4; // Each answered question adds 4 points to max pool
  }

  // Calculate percentage: standard NPQ Disability Index calculation
